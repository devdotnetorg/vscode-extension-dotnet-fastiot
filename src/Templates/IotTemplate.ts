...
export class IotTemplate extends EntityBase<IotTemplateAttribute> {
  ...
  private CreatingMergeDictionary (device:IotDevice, config:IotConfiguration,dstPath:string,nameProject:string,
    ...
    //namespace
    this.MergeDictionary.set("%{project.dotnet.namespace}",<string>dotnetHelper.GetDotNetValidNamespace(nameProject));
    dotnetHelper.GetDotNetValidNamespace(nameProject);
    //template app folder
